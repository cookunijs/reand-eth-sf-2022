...
import {
    IWorldID
} from "@worldcoin/world-id-contracts/src/interfaces/IWorldID.sol";
import {
    ByteHasher
} from "@worldcoin/world-id-contracts/src/libraries/ByteHasher.sol";

/**
 * @title WorldID
 * @notice
 */
abstract contract WorldID {
    ...
    function _decodeProofIdDataByWorldId(
        bytes memory data
    ) internal pure returns (address, uint256, uint256, uint256[8] memory) {
        (
            address input,
            uint256 root,
            uint256 nullifierHash,
        ...
        return (input, root, nullifierHash, proof);
    }
}
